...
def graficar_resultados(resultados):
    ...
    plt.plot(resultados, marker='o', linestyle='-', color='b')  # Dibuja la línea con puntos en cada dato
    plt.title('Pasos necesarios por simulación')  # Título del gráfico
    plt.xlabel('Número de simulación')  # Etiqueta del eje X
    plt.ylabel('Pasos totales para alcanzar la meta')  # Etiqueta del eje Y
    plt.grid(True)  # Activa la cuadrícula para mejor visualización
    plt.xticks(range(len(resultados)))  # Asegura que cada simulación tenga una etiqueta en el eje X
    plt.show(block=False)  # Muestra el gráfico sin bloquear el resto del script
    plt.pause(5)  # Muestra el gráfico por 5 segundos
    plt.close()  # Cierra la figura automáticamente

# Suponiendo que tienes una lista de resultados
    resultados = [100, 120, 110, 105, 98, 130, 112, 117, 121, 115]  # Ejemplo de resultados
            ...
            if mostrar_menu:
                for i, button in enumerate(buttons):
                    if button.collidepoint(mouse_pos):
                        ...
                        graficar_resultados(resultados)  # Llama a Matplotlib para graficar los resultados
